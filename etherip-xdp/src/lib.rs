...
impl EtheripConfig {
    ...
    pub(crate) async fn run_inner(self) -> std::io::Result<()> {
        ...
        let bind_iface = self.bind_iface;
        let bridged_iface = self.bridged_iface;

        let bind_iface_id = net_interfaces::name_to_index(IfName::new(bind_iface.clone())?)?;

        let bind_iface_id = bind_iface_id.inner().ok_or(std::io::Error::other("Unspecified bind iface"))?;

        let bridged_iface_id = net_interfaces::name_to_index(IfName::new(bridged_iface.clone())?)?;

        let bridged_iface_id = bridged_iface_id.inner().ok_or(std::io::Error::other("Bridged iface unspecified"))?;
        ...
        for config in tunnels {
            if !seen_vlans.insert(config.vlan_id.as_u16()) {
                return Err(std::io::Error::other(format!("duplicate tunnel configuration for VLAN {}", config.vlan_id)));
            }
            tunnel_configs.push(config);
        }

        if !seen_vlans.contains(&0u16) {
        ...
        let encap_program: &mut Xdp = ebpf
            ...
            .ok_or_else(|| std::io::Error::other("eBPF program etherip_xdp_encap not found"))?
            .try_into().map_err(|e| std::io::Error::other(e))?;
        encap_program.load().map_err(|e| std::io::Error::other(e))?;
        encap_program.attach(&bridged_iface, XdpFlags::default())
            .map_err(|e| std::io::Error::other(e))?;

        let decap_program: &mut Xdp = ebpf
            .program_mut("etherip_xdp_decap")
            .ok_or_else(|| std::io::Error::other("eBPF program etherip_xdp_decap not found"))?
            .try_into()
            .map_err(|e| std::io::Error::other(e))?;
        decap_program.load().map_err(|e| std::io::Error::other(e))?;
        ...
        let mut if_indexes = aya::maps::HashMap::<_, u32, u32>::try_from(
            ebpf.map_mut("IF_INDEX_MAP")
                .ok_or_else(|| std::io::Error::other("IF_INDEX_MAP not found"))?,
        ).map_err(|e| std::io::Error::other(e))?;

        if_indexes.insert(iface::IF_INDEX_INNER, bridged_iface_id, 0)
            .map_err(|e| std::io::Error::other(e))?;
        if_indexes.insert(iface::IF_INDEX_OUTER, bind_iface_id, 0)
            .map_err(|e| std::io::Error::other(e))?;
        ...
        let default_ipv6_route_prefix =
            Ipv6Net::new(Ipv6Addr::UNSPECIFIED, 0).map_err(|err| std::io::Error::other(err))?;
